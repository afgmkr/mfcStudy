...
BEGIN_MESSAGE_MAP(CmfcCImageDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BTN_RECTANGLE, &CmfcCImageDlg::OnBnClickedBtnRectangle)
END_MESSAGE_MAP()
...
void CmfcCImageDlg::OnBnClickedBtnImage()
{
	...
	//m_image.Save(_T("C:\\Projects\\mfcStudy\\mfcCImage\\Save_Img\\save.bmp"));
}


CString g_strFileImage = _T("C:\\Projects\\mfcStudy\\mfcCImage\\Save_Img\\save.bmp");
...
void CmfcCImageDlg::OnBnClickedBtnRectangle()
{
	int nGray = 90;
...
//	memset(fm, 0xff, sizeof(unsigned char) * nWidth * nHeight);
	...
	int nStartPos = 0;
	int nEndPosX = nWidth - 64;
	int nEndPosY = nHeight - 48;
	int nRandomX = (rand() % (nEndPosX - nStartPos + 1)) + nStartPos;
	int nRandomY = (rand() % (nEndPosY - nStartPos + 1)) + nStartPos;

	for (int j = nRandomY; j < nRandomY + 48; j++) {
		for (int i = nRandomX; i < nRandomX + 64; i++) {
			if (validImgPos(i, j)) {
				fm[(j * nPitch) + i] = nGray;
			}
		}
